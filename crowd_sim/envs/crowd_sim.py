...
class CrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        start_position = np.array(self.robot.get_position())
        end_position = np.array(self.robot.compute_position(action, self.time_step))
        reaching_goal = norm(end_position - np.array(self.robot.get_goal_position())) < self.robot.radius
        start_distance_to_goal = norm(start_position - np.array(self.robot.get_goal_position()))
        end_distance_to_goal = norm(end_position - np.array(self.robot.get_goal_position()))
        new_reward = 2 * (-end_distance_to_goal + start_distance_to_goal) * 0.1
