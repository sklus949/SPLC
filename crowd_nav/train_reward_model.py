...
from reward_model import RewardModel, TransformerRewardModel
import random
import torch
import h5py
import pickle
...
def get_d4rl_dataset():
    file_path = "dataset/dataset.hdf5"
...
def load_queries_with_indices(dataset, num_query, len_query, saved_indices, saved_labels=None, label_type=1,
    ...
    #     print(batch['labels'])
        ...
        human_labels = np.zeros((len(saved_labels), 2))
        human_labels[:, 0] = saved_labels
        human_labels[:, 1] = 1 - saved_labels
        human_labels = human_labels[query_range]
        batch['labels'] = human_labels
        print(batch['labels'])
...
def train(cfg):
    ...
    base_path = "data"
    human_labels_file, human_indices_1_file, human_indices_2_file = sorted(os.listdir(base_path))[2:]
    ...
    if not isinstance(human_indices_1, np.ndarray):
        human_indices_1 = np.array(human_indices_1)
    if not isinstance(human_indices_2, np.ndarray):
        human_indices_2 = np.array(human_indices_2)
    if not isinstance(label_file, np.ndarray):
    ...
    reward_model = TransformerRewardModel(
        ...
        activation="tanh", logger=L, device="cpu")
